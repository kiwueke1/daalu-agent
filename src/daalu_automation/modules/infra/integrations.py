...
class PrometheusAdapter(IntegrationAdapter):
    descriptor = IntegrationDescriptor(
        ...
        description="Pulls active firing alerts from Alertmanager.",
    ...
    async def ingest(self, tenant_id: uuid.UUID) -> int:
        ...
        if not cfg.prometheus_url:
            return 0
        # Alertmanager v2 API — /api/v2/alerts returns active alerts.
        url = cfg.prometheus_url.rstrip("/") + "/api/v2/alerts"
        ...
        for alert in alerts:
            ...
            # Alertmanager's `Watchdog` carries severity "none" and exists
            # only to prove the alerting pipeline is alive — surfacing it
            # as an Alert row is noise, not an incident.
            if severity in ("none", "") or labels.get("alertname") == "Watchdog":
                continue
            # `publish()` coerces severity into the EventSeverity enum
            # (info/warning/critical); clamp any other Alertmanager label
            # value (e.g. "page") to warning so an unknown severity can't
            # crash the event insert and silently drop the alert.
            ...
            await publish(
                EventEnvelope(
                    ...
                    payload={
                        ...
                        # Alertmanager's own label-set hash — `compute_fingerprint`
                        # trusts this verbatim so re-fires collapse onto one Alert
                        # row exactly as Alertmanager groups them.
