...
async def chat_with_tools(
    *,
    ...
    ``tool_choice`` is forwarded to the OpenAI-compatible endpoint. Use
    ``"required"`` to force the model to emit at least one tool call this
    turn (e.g. the first investigative turn, so a model that would rather
    narrate a plan actually gathers evidence); ``"auto"`` lets it decide
    (and produce a final text answer).
