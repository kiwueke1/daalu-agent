...
# rotation invalidates only the affected entry.
...
async def _tenant_kube_or_default(tenant_id: uuid.UUID | None) -> Any:
    ...
    *, namespace: str, pod: str, _tenant_id: uuid.UUID | None = None
    ...
    for c in status.container_statuses or []:
        ...
        if state.running:
            ...
            s = "unknown"
        lines.append(
            f"  - {c.name}: restarts={c.restart_count} ready={c.ready} {s}"
        )
...
async def _list_pods(
    *,
    ...
    for p in pods.items:
        ...
        lines.append(
            f"{p.metadata.name}  {ready}/{total}  {p.status.phase}  "
...
async def _rollout_undo(
    *,
    ...
    body = {
        "spec": {
            "template": {
                "metadata": target.spec.template.metadata.to_dict()
                if target.spec.template.metadata
                else {},
                "spec": target.spec.template.spec.to_dict(),
            }
        }
    }
    await _run(
...
async def _call_external_api(
    *,
    ...
    async with httpx.AsyncClient(timeout=20, headers=headers, verify=verify) as client:
        try:
...
async def execute_tool(
    ...
    *,
    ...
    try:
        ...
        logger.exception("kube_tool.failed", tool=name, input=tool_input)
