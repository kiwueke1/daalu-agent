...
async def overview(
    ...
    return {
        ...
        "metrics_available": prom.configured and view.role != "none",
        "panels": panels,
        "local_inference": {
...
# ── self-hosted GPU onboarding (the UI "Add GPU" wizard) ──────────────────


class GpuDiscoverNodeOut(BaseModel):
    name: str
    gpu_count: int
...
class GpuDiscoverOut(BaseModel):
    reachable: bool
    error: str | None = None
    total_gpus: int = 0
    nodes: list[GpuDiscoverNodeOut] = Field(default_factory=list)
...
class GpuOnboardIn(BaseModel):
    gpu_class: str = Field(..., min_length=1, max_length=64)
    model_classifier: str = Field(..., min_length=1, max_length=255)
    namespace: str = Field("daalu", max_length=253)
    service_url: str | None = Field(None, max_length=512)


class GpuOnboardOut(BaseModel):
    ok: bool
    role: str
    has_gpu: bool
    metrics_available: bool
    dcgm_scrapeable: bool
    warnings: list[str] = Field(default_factory=list)


@router.get("/gpu/discover", response_model=GpuDiscoverOut)
async def gpu_discover(tenant_id: uuid.UUID = Depends(current_tenant_id)):
    """List the GPUs the connected cluster advertises + pre-fill hints for the
    Add-GPU wizard (suggested class, served model, whether DCGM is scraped)."""
    from daalu_automation.core.gpu.local_onboard import discover_gpus

    d = await discover_gpus(tenant_id)
    return GpuDiscoverOut(
        reachable=d.reachable,
        error=d.error,
        total_gpus=d.total_gpus,
        nodes=[
            GpuDiscoverNodeOut(
                name=n.name, gpu_count=n.gpu_count, gpu_product=n.gpu_product,
                gpu_memory=n.gpu_memory, ready=n.ready,
            )
            for n in d.nodes
        ],
        suggested_gpu_class=d.suggested_gpu_class,
        suggested_model=d.suggested_model,
        suggested_service_url=d.suggested_service_url,
        prometheus_connected=d.prometheus_connected,
        dcgm_scrapeable=d.dcgm_scrapeable,
        already_onboarded=d.already_onboarded,
    )


@router.post("/gpu/onboard", response_model=GpuOnboardOut)
async def gpu_onboard(
    body: GpuOnboardIn,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(current_admin),
):
    """Onboard the GPU for this tenant: stamp the tenant-labelled DCGM
    ServiceMonitor and write the gpu_tenants owner row (flips AI Factory to the
    live GPU view). The UI equivalent of scripts/onboard-cluster.sh (a)+(b)."""
    from daalu_automation.core.gpu.local_onboard import GpuOnboardError, onboard_gpu

    try:
        res = await onboard_gpu(
            user.tenant_id,
    ...
    except GpuOnboardError as e:
        raise HTTPException(400, str(e)) from e
    view = await resolve_factory_view(db, user.tenant_id)
    prom = await PrometheusClient.for_tenant(user.tenant_id)
    return GpuOnboardOut(
        ok=res.ok,
        role=view.role,
        has_gpu=view.has_gpu,
        metrics_available=prom.configured and view.role != "none",
        dcgm_scrapeable=res.dcgm_scrapeable,
...
async def gpu_summary(
    ...
    client = await PrometheusClient.for_tenant(tenant_id)
