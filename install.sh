...
if [ ! -f .env ]; then
  cp .env.example .env
  # Generate a unique SECRET_KEY. It signs auth tokens AND derives the at-rest
  # encryption key for stored credentials (core/crypto.py), so even in the
  # single-operator LOCAL_NO_AUTH default it must not stay the shipped
  # placeholder. Prefer openssl; fall back to /dev/urandom.
  if gen="$(openssl rand -hex 32 2>/dev/null)" && [ -n "$gen" ]; then :; else
    gen="$(head -c 32 /dev/urandom 2>/dev/null | od -An -tx1 | tr -d ' \n')"
  fi
  if [ -n "$gen" ]; then
    tmp="$(mktemp)"; sed "s#^SECRET_KEY=.*#SECRET_KEY=${gen}#" .env > "$tmp" && mv "$tmp" .env
    ok "created .env from .env.example  ${DIM}(generated a unique SECRET_KEY)${RST}"
  else
    ok "created .env from .env.example"
    warn "could not auto-generate a SECRET_KEY — set one in .env before enabling auth"
  fi
else
...
printf "\n"
printf "   %s🔒 Authentication is disabled (LOCAL_NO_AUTH=true) and ports are bound to\n" "$YLW"
printf "      127.0.0.1, so this stack is reachable only from this machine. To expose it\n"
printf "      to others, front it with an authenticating reverse proxy and set BIND_ADDR\n"
printf "      in .env — see docs/04-deployment.md §1.6.%s\n\n" "$RST"
# A local GPU k3s cluster won't light up AI Factory until it's onboarded — and
# that's easy to miss once the stack is up (the page just shows local-inference
# mode). Nudge toward the one-shot onboarding script when a GPU node is present.
if command -v kubectl >/dev/null 2>&1 \
   && kubectl get nodes -o json 2>/dev/null | grep -q 'nvidia.com/gpu'; then
  printf "   %s▶ GPU cluster detected%s — light up AI Factory (live GPU view) with:\n" "$BLU" "$RST"
  printf "       %s./scripts/onboard-cluster.sh%s\n\n" "$BOLD" "$RST"
fi
# Existing monitored cluster? The manual demo can run on it directly (no kind):
# up-manual.sh deploys checkout-api onto your current context and prints the
# kubeconfig path + Prometheus/Loki URLs to paste into the UI. Surface those
# values here too so you don't have to hunt for them.
if command -v kubectl >/dev/null 2>&1 \
   && kubectl get crd prometheuses.monitoring.coreos.com >/dev/null 2>&1; then
  printf "   %s▶ Monitored cluster detected%s — run the manual demo on it (no kind cluster):\n" "$BLU" "$RST"
  printf "       %s./demo/up-manual.sh%s   # deploys checkout-api, then prints what to onboard:\n" "$BOLD" "$RST"
  printf "         Kubernetes  → the kubeconfig file it writes (full path printed at the end)\n"
  printf "         Prometheus  → http://host.docker.internal:%s\n" "${PROM_NODEPORT:-30090}"
  printf "         Loki        → http://host.docker.internal:%s\n\n" "${LOKI_NODEPORT:-30310}"
fi
printf "   %sNext:%s\n" "$BOLD" "$RST"
