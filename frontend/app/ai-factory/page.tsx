...
import { GpuOnboardWizard } from "./components/gpu-onboard-wizard";
...
export default function AiFactoryPage() {
  ...
  // The "Add GPU" wizard (self-hosted onboarding) overlay.
  ...
  return (
    <div className="space-y-8 max-w-[1200px]">
        ...
        <div className="space-y-3">
          {isAdmin && (
            <div className="flex justify-end">
              <button
                onClick={() => setShowGpuWizard(true)}
                className="inline-flex items-center gap-1.5 text-xs h-8 px-3 rounded-lg border border-accent-cyan/40 text-accent-cyan hover:bg-accent-cyan/10"
              >
                <Cpu className="h-3.5 w-3.5" /> Add GPU
              </button>
            </div>
          )}
      ...
      {role === "none" && !localConfigured && !overview.isLoading && (
        <section className="rounded-xl border border-line bg-[color:var(--bg-elevated)]/40 p-8 text-center">
          ...
          <p className="text-xs text-muted mt-1.5 max-w-[480px] mx-auto">
            ...
            cluster under Managed Infra, then onboard the GPU here — the factory
          ...
          <div className="mt-4 flex items-center justify-center gap-2">
            {isAdmin && (
              <button
                onClick={() => setShowGpuWizard(true)}
                className="inline-flex items-center gap-1.5 text-xs h-8 px-3 rounded-lg bg-gradient-to-r from-accent-emerald to-accent-cyan text-bg-base font-medium"
              >
                <Cpu className="h-3.5 w-3.5" /> Add GPU
              </button>
            )}
            <Link
              href="/managed-infra"
              className="inline-flex items-center gap-1.5 text-xs h-8 px-3 rounded-lg border border-line text-muted hover:text-fg"
            >
              Managed Infra <ArrowUpRight className="h-3.5 w-3.5" />
            </Link>
          </div>
        </section>
      )}
      ...
      {showGpuWizard && (
        <GpuOnboardWizard onClose={() => setShowGpuWizard(false)} />
      )}
    </div>
  );
}
