...
// Providers that have a read-only detail console at /observability/{provider}
// (the metrics/logs analogue of the kubectl console). OpenSearch isn't wired
// to a console yet, so its row stays Connect/Edit-only.
const OBSERVABILITY_CONSOLE_PROVIDERS = new Set<string>([
  "prometheus",
  "thanos",
  "loki",
]);

// Providers a tenant may connect more than once (one row per named instance).
// Mirrors the backend's _MULTI_INSTANCE_PROVIDERS. For these we never collapse
// rows by provider — each instance is edited/deleted by its own id.
const MULTI_INSTANCE_PROVIDERS = new Set<string>(["kubernetes"]);
...
export default function ManagedInfraPage() {
  ...
  // The specific row being edited (for multi-instance providers). undefined =
  // create new / fall back to the provider's single row.
  const [activeExisting, setActiveExisting] = useState<
  ...
  // Disconnect a provider integration (prometheus / loki / kubernetes / …):
  // deletes the saved config so the row reverts to "not connected" and can be
  // re-added from scratch — the quickest way to re-test the connect/onboard
  // flow (e.g. uploading a kubeconfig). clusterId picks one of several rows
  // for multi-instance providers (kubernetes).
  const removeConfig = useMutation({
    mutationFn: (vars: { provider: string; clusterId?: string }) =>
      api.integrations.deleteConfig(vars.provider, vars.clusterId),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["integrations", "config"] }),
  });

  const handleDeleteConfig = (step: IntegrationStep) => {
    if (!step.provider) return;
    if (
      confirm(
        `Disconnect ${step.title}? Daalu deletes the saved credentials and stops reading from it until you reconnect.`
      )
    ) {
      removeConfig.mutate({ provider: step.provider });
    }
  };

  // Open the connect/edit modal. Pass `existing` to edit a specific row
  // (needed for multi-instance kubernetes, where one provider has many rows).
  const openConnect = (step: IntegrationStep, existing?: IntegrationConfig) => {
  ...
  // The kubeconfig step is reused for every cluster — a tenant can register
  // several `kubernetes` rows, one per cluster (keyed by name). The WireGuard
  // tunnel list below is a separate path needing the cluster-tunnel backend.
  const kubeStep: IntegrationStep | undefined = STEPS_BY_ID["kubernetes"];
  const kubeClusters: IntegrationConfig[] = (integrationConfigs ?? []).filter(
    (c) => c.provider === "kubernetes"
  );
    ...
    <div className="space-y-8 max-w-[1200px]">
      ...
      <section className="space-y-3">
        ...
        <ProviderTable
          ...
          onConnect={(step) => openConnect(step)}
      ...
      <section className="space-y-3">
        <SectionHeader
          title="Observability"
          subtitle="Customer-side alerting, logging, and long-history metric stacks. Status flips to connected after the agent successfully queries the endpoint. Open a connected metrics or logs store to browse it with ready-made queries."
        />
        <ProviderTable
          ...
          onConnect={(step) => openConnect(step)}
          onDelete={handleDeleteConfig}
          detailHref={(step, existing) =>
            existing && OBSERVABILITY_CONSOLE_PROVIDERS.has(step.provider ?? "")
              ? `/observability/${step.provider}`
              : null
          }
        />
      </section>
          ...
          <div className="space-y-3">
            <div className="flex items-start justify-between gap-4">
              <p className="text-xs text-muted max-w-[760px]">
                Daalu reads each cluster with a kubeconfig you paste. Its
                read-only kubectl tools inspect pods, events, and logs during
                triage and apply approved changes. Add as many clusters as you
                operate — each has its own name, and an alert tagged with that
                name routes the agent to the matching cluster.
              </p>
              {kubeStep && (
                <button
                  onClick={() => openConnect(kubeStep)}
            ...
            {kubeClusters.length === 0 ? (
              <div className="rounded-2xl border border-line bg-bg-card px-4 py-8 text-center text-muted text-sm">
                No clusters yet. Click <em>Add cluster</em> to paste or upload a
                kubeconfig.
              </div>
            ) : (
                  ...
                  <tbody>
                    {kubeClusters.map((c) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <ProviderStatusBadge existing={c} />
                        </td>
                        <td className="px-4 py-3 text-right">
                          <div className="inline-flex items-center gap-2">
                            <Link
                              href={`/clusters/kubeconfig?cluster=${c.id}`}
                              className="text-xs h-8 px-3 rounded-lg border border-line text-muted hover:text-accent-cyan hover:border-accent-cyan/40 inline-flex items-center gap-1.5"
                            >
                              Open <ChevronRight className="h-3.5 w-3.5" />
                            </Link>
                            <button
                              type="button"
                              onClick={() => kubeStep && openConnect(kubeStep, c)}
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (
                                  confirm(
                                    `Disconnect cluster "${c.name}"? Daalu deletes its saved kubeconfig and stops reading from it until you reconnect.`
                                  )
                                ) {
                                  removeConfig.mutate({
                                    provider: "kubernetes",
                              ...
                              className="text-xs h-8 px-2 rounded-lg border border-line text-muted hover:text-red-500 hover:border-red-500/40 inline-flex items-center"
                            >
      ...
      {activeStep && (
        <ConnectModal
          step={activeStep}
          existing={
            // A specific row when editing (multi-instance), else the provider's
            // single row, else undefined (creating a new instance). Never fall
            // back to a collapsed row for multi-instance providers, or "Add
            // cluster" would pre-fill from an existing cluster.
            activeExisting ??
            (activeStep.provider &&
            !MULTI_INSTANCE_PROVIDERS.has(activeStep.provider)
...
function ProviderTable({
  ...
  // When supplied, connected rows get a Disconnect (trash) action that
  // deletes the saved config so the provider can be re-added from scratch.
  onDelete?: (step: IntegrationStep) => void;
  // When supplied and it returns a path for a connected provider, the row
  // gets an "Open" link into a detail console (e.g. /observability/loki).
  detailHref?: (
    step: IntegrationStep,
    existing: IntegrationConfig | undefined
  ) => string | null;
            ...
            return (
              <tr key={step.id} className="border-t border-line">
                ...
                <td className="px-4 py-3 text-right">
                  <div className="inline-flex items-center gap-2">
                    {(() => {
                      const href = detailHref?.(step, existing);
                      return href ? (
                        <Link
                          href={href}
                          className="text-xs h-8 px-3 rounded-lg border border-line text-muted hover:text-accent-cyan hover:border-accent-cyan/40 inline-flex items-center gap-1.5"
                    ...
                    {existing && onDelete && (
                      <button
                        ...
                        title="Disconnect"
                        aria-label={`Disconnect ${step.title}`}
                        className="text-xs h-8 px-2 rounded-lg border border-line text-muted hover:text-red-500 hover:border-red-500/40 inline-flex items-center"
                      >
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
