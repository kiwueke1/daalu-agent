...
export function InvestigationTiles({ alert }: InvestigationTilesProps) {
  ...
  const { data: messages, isLoading } = useQuery({
    ...
    refetchInterval: (q) => {
      ...
      // i.e. an investigation is actively in progress. Stops once the pass has
      // settled (summary landed or a remediation is queued for approval) — see
      // investigationSettled — so a transcript ending on a batched tool result
      // can't make us poll "Triaging…" forever.
      const last = msgs[msgs.length - 1];
      const inFlight =
        !!last &&
        (last.role === "user" || last.role === "tool") &&
        !investigationSettled(msgs);
  ...
  // step (an investigation actively in progress). Bounded — flips off once the
  // pass has settled (investigationSettled: summary produced or a remediation
  // queued for approval), so neither a malformed answer nor a final turn that
  // ends on a batched tool result can pin the spinner on forever.
  const msgList = messages ?? [];
  const lastMsg = msgList[msgList.length - 1];
  const working =
    triage.isPending ||
    (!!lastMsg &&
      (lastMsg.role === "user" || lastMsg.role === "tool") &&
      !investigationSettled(msgList));
...
function RemediationTile({
  ...
  working: boolean;
  activity: ActivityStep[];
  messages: AlertChatMessage[];
}) {
  const [open, setOpen] = useState(false);
  const hasActivity = activity.length > 0;
  const qc = useQueryClient();
  // message id → its text, so each action card can show the agent's own
  // explanation (from the message that proposed it).
  const contentById = useMemo(() => {
    const m = new Map<string, string>();
    for (const msg of messages) if (msg.content) m.set(msg.id, msg.content);
    return m;
  }, [messages]);
  const router = useRouter();
  // Approving runs the remediation as a Workflow; jump to its page so the
  // operator watches the plan execute step by step.
  const approveOne = useMutation({
    mutationFn: (actionId: string) => api.alerts.chat.approve(alert.id, actionId),
    onSuccess: (data) => {
      qc.setQueryData(["alert-chat", alert.id], data.messages);
      router.push(`/workflows/${data.workflow_run_id}`);
    },
  });
  ...
  // fans out, then opens the first workflow it created.
  ...
  const approvePlan = async () => {
    ...
    try {
      for (const a of pendingWrites) {
        const res = await api.alerts.chat.approve(alert.id, a.id);
        if (!firstRunId) firstRunId = res.workflow_run_id;
      }
      qc.invalidateQueries({ queryKey: ["alert-chat", alert.id] });
    } finally {
      setBulkApproving(false);
    }
    if (firstRunId) router.push(`/workflows/${firstRunId}`);
  };
                    ...
                    >
                      <div className="flex items-center gap-2 text-[12px]">
                        ...
                        <span className="text-muted font-mono text-[11px] truncate">
                          {argTarget(a.tool_input)}
                        </span>
                      </div>
                      {/* What this action is trying to do — the agent's own
                          words when available, else a mechanical summary.
                          Wraps (no truncation) so the operator can read it. */}
                      <div className="text-[12px] text-[color:var(--text)]/80 mt-1.5 whitespace-pre-wrap">
                        {actionRationale(a, contentById) ?? describeAction(a)}
                      </div>
                      {/* The exact command, expandable so the full patch is
                          available without dominating the card. */}
                      <details className="mt-1.5 group">
                        <summary className="cursor-pointer text-[11px] text-muted hover:text-[color:var(--text)] inline-flex items-center gap-1">
                          <ChevronRight className="h-3 w-3 transition-transform group-open:rotate-90" />
                          Show exact command
                        </summary>
                        <pre className="mt-1 text-[11px] font-mono bg-bg-base/60 border border-line rounded p-2 overflow-x-auto whitespace-pre-wrap break-all">
                          {a.tool_name}({JSON.stringify(a.tool_input, null, 2)})
                        </pre>
                      </details>
...
/**
 * True once an autonomous pass has actually terminated, judged from the
 * transcript alone.
 *
 * The naive signal — "the latest message is a tool result, so the model still
 * owes us a next step" — is unreliable. The server's investigation loop returns
 * as soon as it proposes a write action (which then waits for approval), but a
 * write tool_use appends NO message while read tool_uses in the same turn DO.
 * So when the model batches reads alongside its remediation in the final turn
 * (e.g. `[get_logs, get_events, patch_resource]`), the transcript ends on a
 * read's tool result even though the pass is over. Polling on the naive signal
 * then never stops and the button shows "Triaging…" forever.
 *
 * Treat the pass as settled when either a write action is pending approval, or
 * the model has produced its closing summary — both mean the loop has returned.
 */
function investigationSettled(messages: AlertChatMessage[]): boolean {
  const hasPendingWrite = messages.some((m) =>
    (m.actions || []).some(
      (a) => a.status === "pending" && a.requires_approval,
    ),
  );
  if (hasPendingWrite) return true;
  const summary = parseSections(latestSummary(messages));
  return !!(summary.rootCause || summary.background || summary.remediationPlan);
}
...
function describeAction(a: AlertAction): string {
  ...
  switch (a.tool_name) {
    ...
    case "patch_resource": {
      const kind = (typeof i.kind === "string" ? i.kind : "resource").toLowerCase();
      const fields = patchContainerFields(i.patch);
      return fields
        ? `Patch ${kind} ${ns}/${name} — ${fields}.`
        : `Strategic-merge patch ${kind} ${ns}/${name}.`;
    }
...
/**
 * Summarize what a strategic-merge patch actually changes, with the new values
 * — e.g. "set memory request 256Mi, limit 512Mi on container 'app'" rather than
 * a bare "set resources" — so the operator can tell what a patch_resource action
 * does without reading the raw JSON. Returns null when the patch isn't the usual
 * deployment/container shape.
 */
function patchContainerFields(patch: unknown): string | null {
  try {
    const containers = (patch as Record<string, any>)?.spec?.template?.spec
      ?.containers;
    if (Array.isArray(containers) && containers.length) {
      return containers
        .map((c: Record<string, any>) => {
          const changes = Object.keys(c)
            .filter((k) => k !== "name")
            .map((k) => describeContainerField(k, c[k]))
            .filter(Boolean);
          if (!changes.length) return null;
          return `${changes.join("; ")} on container '${c.name ?? "?"}'`;
        })
        .filter(Boolean)
        .join("; ");
    }
...
/**
 * Human phrase for one changed container field, including the new value(s) so
 * the operator sees the actual change. `resources` is grouped by resource name
 * ("memory request 256Mi, limit 512Mi") since that's the common remediation;
 * other fields fall back to naming the field.
 */
function describeContainerField(key: string, val: any): string {
  switch (key) {
    case "resources": {
      const req = (val?.requests ?? {}) as Record<string, string>;
      const lim = (val?.limits ?? {}) as Record<string, string>;
      // One clause per resource (memory, cpu): "memory request X, limit Y".
      const names = Array.from(
        new Set([...Object.keys(req), ...Object.keys(lim)])
      );
      const parts = names.map((n) => {
        const bits: string[] = [];
        if (req[n] != null) bits.push(`request ${req[n]}`);
        if (lim[n] != null) bits.push(`limit ${lim[n]}`);
        return `${n} ${bits.join(", ")}`;
      });
      return parts.length ? `set ${parts.join("; ")}` : "set resources";
    }
    case "image":
      return `set image to ${val}`;
    case "command":
    case "args":
      return `override the container ${key}`;
    case "env": {
      const names = Array.isArray(val)
        ? val.map((e: any) => e?.name).filter(Boolean)
        : [];
      return names.length ? `set env ${names.join(", ")}` : "set env";
    }
    default:
      return `set ${key}`;
  }
...
/**
 * The agent's own explanation for a proposed action — the prose it wrote in the
 * message that proposed it (each "[system]" step explains one tool call). This
 * is the real "what is this trying to do", far better than the mechanical
 * describeAction() fallback. Returns null if we can't find it.
 */
function actionRationale(
  a: AlertAction,
  byId: Map<string, string>
): string | null {
  const content = byId.get(a.message_id)?.trim();
  if (!content) return null;
  // Steps are narrated as "[system] <explanation> ... (And call <tool>(…))".
  const blocks = content.split(/\n(?=\[system\])/i);
  const block =
    blocks.find((b) => b.includes(`${a.tool_name}(`)) ??
...
/** Short target identifier for an action's header (namespace/name). */
function argTarget(input: Record<string, unknown>): string {
  const ns = typeof input.namespace === "string" ? input.namespace : null;
  const name = typeof input.name === "string" ? input.name : null;
  if (ns && name) return `${ns}/${name}`;
  if (name) return name;
  const scalar = Object.entries(input).find(([, v]) => typeof v !== "object");
  return scalar ? `${scalar[0]}=${String(scalar[1])}` : "";
}
