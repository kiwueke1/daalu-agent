...
export const api = {
  ...
  workflows: {
    ...
    runs: (opts: { module?: string; source?: "alert" } = {}) => {
      const qs = new URLSearchParams();
      if (opts.module) qs.set("module", opts.module);
      if (opts.source) qs.set("source", opts.source);
      const q = qs.toString();
      return request<WorkflowRun[]>(`/workflows/runs${q ? `?${q}` : ""}`);
    },
  ...
  integrations: {
    ...
    putConfig: (
      ...
      payload: {
        ...
        name?: string;
        // Multi-instance providers (kubernetes): pass an existing row's id to
        // edit it; omit to create a new named instance. Single-instance
        // providers ignore it and upsert their one row.
        cluster_id?: string;
    ...
    // clusterId selects which instance to delete for multi-instance
    // providers (kubernetes); omit for single-instance providers.
    deleteConfig: (provider: string, clusterId?: string) =>
      request<void>(
        `/integrations/config/${provider}${clusterId ? `?cluster_id=${clusterId}` : ""}`,
        { method: "DELETE" }
      ),
  },
  infra: {
  ...
  observability: {
    overview: (provider: string) =>
      request<ObservabilityOverview>(`/observability/${provider}/overview`),
    catalog: (provider: string) =>
      request<ObsQuerySpec[]>(`/observability/${provider}/catalog`),
    runQuery: (provider: string, body: ObsQueryRequest) =>
      request<{ results: ObsQueryResult[] }>(`/observability/${provider}/query`, {
        method: "POST",
        body: JSON.stringify(body),
      }),
  },
  ...
  aiFactory: {
    ...
    // Self-hosted "Add GPU" wizard: discover the cluster's GPUs, then onboard.
    discoverGpu: () => request<GpuDiscovery>("/ai-factory/gpu/discover"),
    onboardGpu: (body: GpuOnboardRequest) =>
      request<GpuOnboardResult>("/ai-factory/gpu/onboard", {
        method: "POST",
        body: JSON.stringify(body),
      }),
...
// ── Add-GPU wizard (self-hosted onboarding) ───────────────────────────────

export interface GpuDiscoverNode {
  name: string;
  gpu_count: number;
...
export interface GpuDiscovery {
  reachable: boolean;
  error: string | null;
  total_gpus: number;
  nodes: GpuDiscoverNode[];
  suggested_gpu_class: string | null;
  suggested_model: string | null;
  suggested_service_url: string | null;
  prometheus_connected: boolean;
  dcgm_scrapeable: boolean;
  already_onboarded: boolean;
}

export interface GpuOnboardRequest {
  gpu_class: string;
  model_classifier: string;
  namespace?: string;
...
export interface GpuOnboardResult {
  ok: boolean;
  role: AiFactoryRole;
  has_gpu: boolean;
  metrics_available: boolean;
...
// ── Observability console (Prometheus/Thanos metrics, Loki logs) ──────────

export interface ObsQuerySpec {
  id: string;
  label: string;
  group: string;
  query: string;
...
export interface ObservabilityOverview {
  ...
  logs: LogsOverview | null;
}

export interface ObsQueryRequest {
  query_ids: string[];
  // metrics
