...
MEM_LIMIT="${MEM_LIMIT:-24Gi}"                # pod memory ceiling (raise for big models, e.g. 48Gi)
RUNTIME_CLASS="${RUNTIME_CLASS:-auto}"        # k8s RuntimeClass for GPU access ("auto" detects
                                              #   "nvidia"; set to "" to disable, or a custom name)
TOOL_PARSER="${TOOL_PARSER:-}"                # vLLM tool-call parser → enables agentic tool use
                                              #   (e.g. qwen3_coder, hermes, llama3_json, mistral)
...
# Optional native tool-calling. Agent models (the InfraAgent's kube-tool loop)
# need vLLM to PARSE tool calls into OpenAI `tool_calls`; without the parser the
# model emits them as plain text and the agent never sees a tool call. Each model
# family has its own parser (Qwen3-Coder → qwen3_coder, Hermes → hermes, …).
TOOL_ARGS=""
if [ -n "$TOOL_PARSER" ]; then
  TOOL_ARGS=$'\n            - --enable-auto-tool-choice\n            - --tool-call-parser='"$TOOL_PARSER"
fi

# GPU access. The NVIDIA GPU Operator on k3s exposes GPUs ONLY to pods that
# request the `nvidia` RuntimeClass — it is NOT the default runtime, so a pod
# without it boots with no CUDA driver ("Failed to infer device type"). Detect
# the RuntimeClass and pin it; RUNTIME_CLASS="" opts out (host-driver clusters
# where nvidia already IS the default runtime).
if [ "$RUNTIME_CLASS" = "auto" ]; then
  if kubectl get runtimeclass nvidia --request-timeout=4s >/dev/null 2>&1; then
...
RTC_LINE=""
[ -n "$RUNTIME_CLASS" ] && RTC_LINE="      runtimeClassName: ${RUNTIME_CLASS}"
...
${HF_ENV}
          ...
          resources:
            limits: { nvidia.com/gpu: ${GPU_COUNT}, memory: ${MEM_LIMIT} }
